...
async function pocketmarket() {
  ...
  app.use(helmet({ contentSecurityPolicy: (process.env.NODE_ENV === 'prod') ? undefined : false })); // set security-related HTTP headers. Protect app from some well-known web vulnerabilities
